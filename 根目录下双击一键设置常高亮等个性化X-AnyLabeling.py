...
print(f"开始处理 shape.py 路径：{shape_path}")
...
modified_count_selected = 0
modified_count_fill = 0
with open(shape_path, "w", encoding="utf-8") as f:
    for line in lines:
        if "self.selected = False" in line:
            line = line.replace("False", "True")
            modified_count_selected += 1
            print("✔ 修改 self.selected = False → True")
        elif "self.fill = False" in line:
            line = line.replace("False", "True")
            modified_count_fill += 1
            print("✔ 修改 self.fill = False → True")
        f.write(line)

print(f"shape.py 修改完成：共修改 self.selected {modified_count_selected} 处，self.fill {modified_count_fill} 处\n")
