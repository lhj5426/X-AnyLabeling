...
def export_ocr_subtitle(self, fmt="srt"):
    """导出 OCR 标注为 SRT 或 ASS 字幕"""
    import re
    ...
    ts_pattern = re.compile(
        r"(\d{1,2}_\d{2}_\d{2}_\d{3})__(\d{1,2}_\d{2}_\d{2}_\d{3})",
        re.IGNORECASE,
    )
    ...
    def extract_texts_from_data(data):
        """从 X-AnyLabeling 标注 JSON 中提取字幕文本（按 shapes 列表顺序 = 标注序号）"""
    ...
    for dirpath, _, filenames in os.walk(image_dir):
        for fname in filenames:
            ...
            start_sec = parse_ts(match.group(1))
            end_sec = parse_ts(match.group(2))
            subtitles.append((start_sec, end_sec, "".join(texts)))

    if not subtitles:
        popup = Popup(
            self.tr("没有找到可导出的 OCR 字幕数据"),
            self,
            icon=new_icon_path("warning", "svg"),
        )
        popup.show_popup(self, position="center")
    ...
    save_path, _ = QFileDialog.getSaveFileName(
        self, f"导出 {fmt.upper()} 字幕", image_dir, f"{fmt.upper()} Files (*{ext})"
    )
    if not save_path:
        return
    ...
    try:
        if fmt == "srt":
            ...
            for i, (start, end, text) in enumerate(subtitles, 1):
                lines.append(f"{i}")
                lines.append(f"{ts_to_srt(start)} --> {ts_to_srt(end)}")
                lines.append(text)
                lines.append("")
            content = "\n".join(lines)
        else:
            ass_header = """[Script Info]
; Script generated by X-AnyLabeling (OCR subtitle export)
; Inspired by Arctime Pro 4.5 — http://arctime.cn/
...
Style: Default,微软雅黑,54,&H00FFFFFF,&H00FFFFFF,&H19533B3B,&H910E0807,0,0,0,0,100.0,100.0,0.0,0.0,1,3.1,1.2,2,135,135,0,1
...
Style: Yahai-4K,微软雅黑,120,&H00FFFFFF,&H00FFFFFF,&H16534241,&HE12E2625,0,0,0,0,100.0,100.0,0.0,0.0,1,4.5,2.5,2,200,200,110,1
...
Style: BarFX,微软雅黑,18,&H00FFFBDB,&H000000FF,&H9E49FF16,&H66383726,0,0,0,0,100.0,100.0,0.0,0.0,1,2.0,1.0,2,0,0,4,1

[Events]
Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text"""
            lines = [ass_header]
            for i, (start, end, text) in enumerate(subtitles):
                layer = 2 if i % 2 == 0 else 1
                safe_text = text.replace("\\", "\\\\").replace("{", "\\{").replace("}", "\\}")
                lines.append(
                    f"Dialogue: {layer},{ts_to_ass(start)},{ts_to_ass(end)},"
                    f"Default,atg1,0,0,0,,{safe_text}"
                )
            content = "\n".join(lines) + "\n"
        ...
        popup.show_popup(self, popup_height=50, position="center")
    except Exception as e:
