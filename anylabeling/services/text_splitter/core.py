...
def split_blocks_into_lines(
    image: np.ndarray,
    rects: list[tuple[int,int,int,int]],  # [(x1,y1,x2,y2), ...]
    source_language: str = "",
) -> list[dict]:
    """对多个矩形区域做行分割，返回 [{bbox, lines, direction}, ...]"""
    if image is None or image.size == 0 or not rects:
        return []
    ...
    for rect in rects:
        x1, y1, x2, y2 = rect
        crop_box = _clamp_box(_expand_box([x1, y1, x2, y2], 4, 4, width, height), width, height)
        cx1, cy1, cx2, cy2 = crop_box
        if cx2 <= cx1 or cy2 <= cy1:
            results.append({"bbox": rect, "lines": [rect], "direction": _fallback_direction(crop_box, source_language)})
            continue

        crop = image[cy1:cy2, cx1:cx2]
        lines, direction = _detect_lines_and_direction_in_crop(crop, source_language)
        image_lines = [_offset_line(line, cx1, cy1) for line in lines]
        if direction == "vertical":
            image_lines = _align_vertical_lines_to_detector_block(image_lines, [x1, y1, x2, y2])

        image_lines = _sort_lines(image_lines, direction)
        results.append({"bbox": rect, "lines": image_lines, "direction": direction})
