...
class OCR(nn.Module):
    ...
    def infer_color_only(self, img: torch.FloatTensor, img_widths: List[int]):
        """Fast color-only inference: backbone + encoder + 1 decoder step.
        Skips beam search entirely — only extracts fg/bg colors.
        Returns (fg_pred, bg_pred, fg_ind_pred, bg_ind_pred) tensors."""
        N, _, _, _ = img.shape
        dev = img.device
        ...
        # Encoder mask: mask out padded width positions
        W = memory.shape[1]
        max_w = max(img_widths)
        encoder_mask = torch.zeros((N, W), dtype=torch.bool, device=dev)
        for i in range(N):
            valid_w = max(img_widths[i] // 4, 1)  # backbone downsamples by 4
            if valid_w < W:
                encoder_mask[i, valid_w:] = True

        # Encoder
        memory = self.encoder_forward(memory, encoder_mask)

        # Single decoder step with <S> start token
        start_tok = 1  # <S>
        start_idx = torch.full((N, 1), start_tok, dtype=torch.long, device=dev)
        char_embd = self.embd(start_idx)  # N, 1, D

        decoded = char_embd
        for layer in self.decoders:
            decoded = layer(decoded, memory, memory_key_padding_mask=encoder_mask)

        color_feats = self.color_pred1(decoded)  # N, 1, 64
        return (
            self.color_pred_fg(color_feats).squeeze(1),      # N, 3
            self.color_pred_bg(color_feats).squeeze(1),      # N, 3
            self.color_pred_fg_ind(color_feats).squeeze(1),  # N, 2
            self.color_pred_bg_ind(color_feats).squeeze(1),  # N, 2
        )
